...
export const login = (email:any, password:any) => async (dispatch:any) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_BASE_SERVER_URL}/api/users/login`, 
      { email, password },
      config
    );    
...
export const register = (name:any, email:any, password:any) => async (dispatch:any) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_BASE_SERVER_URL}/api/users`,
      { name, email, password },
      config
    );
...
export const getUserDetails = (id:any) => async (dispatch:any, getState:any) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.NEXT_PUBLIC_BASE_SERVER_URL}/api/users/${id}`, config);
...
export const updateUserProfile = (user:any) => async (dispatch:any, getState:any) => {
  try {
    ...
    const { data } = await axios.put(`${process.env.NEXT_PUBLIC_BASE_SERVER_URL}/api/users/profile`, user, config);
